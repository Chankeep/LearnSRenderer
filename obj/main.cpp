...
struct PhongShader : public IShader
{
    ...
    {
	    const Vec2f uv = uvs * bar;
        const Vec3f bn = (Varying_normals * bar).normalize();
        Vec4f sb_pos = Mshadow * embed<4>(Varying_pos * bar);
        sb_pos = sb_pos / sb_pos[3];

	    int x = static_cast<int>(sb_pos[0]);
	    int y = static_cast<int>(sb_pos[1]);
        mat<3, 3, float> A;
        ...
        float shadow = .3 + .7 * (shadowbuffer[x+y*width] < sb_pos[2] + 20);
        Vec3f reflect = (normal * (normal * light_dir * 2.f) - light_dir).normalize();
        float diff = std::max(0.f, std::min(255.f, normal * light_dir));
        float spec = pow(std::max(0.f, reflect.z), model->specular(uv)+1);
        TGAColor c = model->diffuse(uv);
        color = c;
        for (int i = 0; i < 3; i++)
            color[i] = std::min<float>(12 + c[i] * shadow * (0.8 * diff + spec), 255);
